...
if WL.cfg.OculusMonitorView % EAC: for additional window
...
while ~Exit % && test_count < length(delays)
    ...
    %WL.Timer.Graphics.idle_loop.Loop;
    ...
    if WL.cfg.OculusMonitorView && wl_everyhz(WL.cfg.OculusMonitorUpdateHz)
...
%             end
            ...
            if ~WL.cfg.OculusRift
                ...
                for eyeIndex = 0:1 % left eye (0) then right eye (1)
                    ...
                    % EAC: Either this ^^ (and in properties of experiment)
                    % Or we could pass it in as a parameter of display_func (probably better)
