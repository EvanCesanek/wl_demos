classdef ObjectLiftFromAbove < wl_experiment_v1_8
    ...
    methods
        ...
        function my_initialise(WL, varargin)
            ...
            % Shaders
            WL.init_shader_program(vspath,fspath);
            
            % Camera
            WL.init_camera();
            
            % Lights (% w = 0 for directional light, w = 1 for point light)
            WL.Light.Position =  [ -1  -5  10  0;]; % Remember to change NR_LIGHTS in frag shader if you add lights
            ...
            WL.Light = rmfield(WL.Light, 'Shadows'); % Comment out if you want to have shadows
            WL.init_lighting();
...
%             WL.printf('FieldState=%d, ok=%d\n',WL.FieldState,ok);
        ...
        function display_func(WL)
            ...
            % In v1.8+ these are already called in wl_main_loop
        ...
        function [] = init_shader_program(WL,vspath,fspath)
            global GL
            Screen('BeginOpenGL', WL.Screen.window);
